...
@LargeTest
public class StudentCoursesDoNotOverlapTest
{
    @Rule
    ...
    public void setup()
    {
        ...
        Course testCourseToOverlap = new Course("Hi 1010", "Hi Again", "Teaches you how to say hello with more words", 3, "AAAAAAAAAAA", 1782.25, "Hello");
        ...
        Section testSectionToOverlap = new Section("Hi 1010 - A01", "test instructor", new Section.availableSectionDays[]{Section.availableSectionDays.Monday}, Section.availableSectionTimes.earlyBird, "Remote", 500, 500, "Hi 1010", "AAAAAAAAAAA");
        ...
        if(!courses.contains(testCourseToOverlap))
        {
            coursePersistence.insertCourses(testCourseToOverlap);
        }
        ...
        if(!sections.contains(testSectionToOverlap))
        {
            sectionPersistence.insertSection(testSectionToOverlap);
        }
    }

    @Test
    public void studentCoursesDoNotOverlapTest()
    {
        ...
        onView(withId(R.id.add_courses_btn)).perform(click());
        onData(anything()).inAdapterView(withId(R.id.categories_list_in_add_courses_fragment)).atPosition(0).perform(click());
        onData(anything()).inAdapterView(withId(R.id.courses_list_in_add_courses)).atPosition(1).perform(click());
        onData(anything()).inAdapterView(withId(R.id.sections_list_to_be_added)).atPosition(0).perform(click());
        onView(withId(R.id.add_section_btn)).perform(click());


        onData(anything())
                .inAdapterView(withId(R.id.student_enrolled_courses))
                .atPosition(0)
                .onChildView(withId(R.id.section_name_list_item))
                .check(matches(withText(containsString("Hello 101 - A01"))));
